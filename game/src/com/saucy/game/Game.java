...
public class Game extends Application {
	...
	public static final boolean DEVELOPER_MODE = false, RESET_MODE = false;
...
//			playBackgroundMusic(true); // True because the background Music should be looped.
	...
	protected void initializeAssets() {
		...
		if (RESET_MODE) {
			...
			user.setCoinCount(0);
			user.setHighScore(0);
			game.settings.preferences().putInteger(Settings.LAUNCHES, 0);
			game.settings.preferences().flush();
			user.save();
		}
	}
